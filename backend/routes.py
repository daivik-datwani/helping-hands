...
def init_app(app):
    ...
    def get_all_users():
        ...
        try:
            ...
            db.close()
        data = {
            "seniors": [{"id": s.id, "name": s.name, "age": s.age, "email": s.email, "phone": s.phone, "password_hash": s.password_hash} for s in seniors],
            "caretakers": [{"id": c.id, "name": c.name, "age": c.age, "email": c.email, "phone": c.phone, "password_hash": c.password_hash} for c in caretakers],
            "help_requests": [{"id": r.id, "senior_id": r.senior_id, "title": r.title, "description": r.description, "category": r.category, "location": r.location, "status": r.status, "created_at": r.created_at} for r in hr]
        }
        return jsonify(data)
